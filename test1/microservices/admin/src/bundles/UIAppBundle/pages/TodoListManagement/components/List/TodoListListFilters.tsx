...
import { TodoListListFiltersForm } from "../../config/TodoListListFiltersForm";

type TodoListListFiltersProps = {
  onUpdate: (filters: any) => void;
};

export const TodoListListFilters = React.memo(TodoListListFiltersBase);

export function TodoListListFiltersBase(props: TodoListListFiltersProps) {
  ...
  const filterForm = use(TodoListListFiltersForm, { transient: true });
  filterForm.build();
