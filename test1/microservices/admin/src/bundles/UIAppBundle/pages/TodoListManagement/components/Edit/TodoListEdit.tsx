...
import { TodoListEditForm } from "../../config/TodoListEditForm";
...
export function TodoListEdit(props: { id: string }) {
  const UIComponents = useUIComponents();
  const t = useTranslate();
  const form = use(TodoListEditForm, { transient: true });
  ...
  const {
    ...
    TodoListEditForm.getRequestBody()
  );
  ...
  return (
    <UIComponents.AdminLayout>
      <Ant.PageHeader
        title={t("management.todo_list.edit.header")}
        ...
        extra={
          features.view
            ? [
                <Link
                  key="view"
                  to={router.path(Routes.TODO_LIST_VIEW, {
                    params: { id: props.id },
                  })}
                >
      ...
      <Ant.Card>
        ...
        {!isLoading && !error && (
          <Ant.Form
            ...
            initialValues={document as TodoList}
