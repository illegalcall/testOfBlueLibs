...
import { TodoListList } from "../../config/TodoListList";

export class TodoListAntTableSmart extends AntTableSmart<TodoList> {
  collectionClass = TodoListCollection;

  getBody(): QueryBodyType<TodoList> {
    return TodoListList.getRequestBody();
  }

  getColumns(): ColumnsType<TodoList> {
    const list = this.container.get(TodoListList);
    list.build();

    return [...list.rest(), this.getActionsColumn()];
  }

  getActionsColumn(): ColumnType<TodoList> {
    return {
  ...
  getSortMap() {
    return TodoListList.getSortMap();
  }
  ...
  getActionItems() {
    ...
    if (features.view) {
      actions.push({
        ...
        icon: <EyeOutlined />,
        action: (model) => {
          this.router.go(Routes.TODO_LIST_VIEW, {
    ...
    if (features.edit) {
      actions.push({
        ...
        icon: <EditOutlined />,
        action: (model) => {
          this.router.go(Routes.TODO_LIST_EDIT, {
            params: { id: model._id.toString() },
          });
        },
      });
